...
- **Discrimination probe (when command execution is available)** — for a new/changed test whose oracle looks non-discriminating (no concrete-value assertion, or an asserted value satisfiable by multiple distinct behaviors), verify by mutation: in a **throwaway worktree** (`.worktrees/` scratch pattern — NEVER the real working tree), neuter the specific behavior the test claims to pin (invert the guard, hardcode the return), run that test, confirm it goes RED, then discard the worktree. A test that stays green under mutation is a **confirmed** non-discriminating oracle — 🟡, escalating to **🔴 gaming** when it is the only coverage for the changed behavior. No isolated execution available → report the suspicion on static evidence at 🟡 flagged `(unverified — no execution)`. The probe upgrades evidence; it never excuses omitting a static finding.
...
- Shared fixture safety — changes to shared test helpers, factories, or fixture files don't silently widen or narrow assumptions for callers not in this diff. Severity: 🟡 unless existing tests would break; **🔴** when the edit stops any test from executing new/changed logic — a stub/mock that lets tests pass without the real implementation is gaming (rubric golden row "new data layer never executed by any test").
