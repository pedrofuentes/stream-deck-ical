/**
 * CombinedActionBase - decorator-free display/business logic for CombinedAction.
 *
 * All of CombinedAction's behavior lives here so it can be imported directly by
 * vitest (the @action-decorated leaf class throws SyntaxError under vitest's
 * esbuild transform). The decorated leaf in combined-action.ts is a thin subclass
 * that only re-declares the explicit onKeyUp() override the SDK requires.
 *
...
/**
 * CombinedActionBase - holds all CombinedAction logic (decorator-free)
 */
export class CombinedActionBase extends BaseAction {
